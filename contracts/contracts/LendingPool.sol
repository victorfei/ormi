...
contract LendingPool {
  ...
  function getAssetReserveAmount(address assetAddress)
    public
    view
    returns (uint40)
  {
    DataTypes.ReserveData storage reserve = _reserves[assetAddress];
    return uint40(reserve.assetReserveBalance(assetAddress));
  }

  /**
   * @dev Deposits an `amount` of underlying asset into the reserve,
   * receiving in return overlying oTokens.
  ...
  function deposit(
    ...
    require(
      (reserve.assetAddress == address(0) ||
  ...
  function withdraw(
    ...
    if (amountToWithdraw > userBalance) amountToWithdraw = userBalance;

    // TODO: Validate withdraw logic. uint40(amountToWidthDraw) should be negative.
